...
if (!jsonlPath || !blueprintUrl || !project) {
  console.error('Usage: node prime-test-session.js <jsonl-path> <blueprint-url> <project> [target-chars]');
  process.exit(1);
}
...
async function main() {
    ...
    try {
      ...
      if (entry.type === 'user' && entry.message?.content) {
        ...
        const text = blocks
          ...
          .join('\n');
        if (text) {
          messages.push({ role: 'assistant', content: text, model: entry.message.model, usage: entry.message.usage });
          totalChars += text.length;
        }
      }
    } catch {}
  ...
  console.log(`Parsed ${messages.length} messages, ${totalChars} chars (~${Math.round(totalChars / 4)}  tokens)`);
    ...
    if (msg.role === 'user') {
      ...
      jsonlLines.push(JSON.stringify({
        ...
        message: {
          ...
          usage: msg.usage || { input_tokens: Math.round(msg.content.length / 4), output_tokens: Math.round(msg.content.length / 4) },
        },
  ...
  console.log(`  docker exec -u hopper blueprint bash -c 'cat /tmp/prime.jsonl >> <session-jsonl-path>'`);
  console.log(`\nOr use SSH:`);
  console.log(`  scp ${outFile} aristotle9@192.168.1.110:/tmp/`);
  console.log(`  ssh aristotle9@192.168.1.110 "docker cp /tmp/prime_${realId.substring(0, 8)}.jsonl blueprint:/tmp/prime.jsonl && docker exec -u hopper blueprint bash -c 'cat /tmp/prime.jsonl >> <session-jsonl-path>'"`);
}
