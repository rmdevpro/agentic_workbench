...
module.exports = function createWatchers({ db, safe, config, sessionUtils, sessionWsClients, checkCompactionNeeds, tmuxName, tmuxExists, CLAUDE_HOME, logger }) {
    ...
    fs.watchFile(jsonlPath, { persistent: false, interval: 2000 }, () => {
      ...
      jsonlDebounceTimers.set(tmuxSession, setTimeout(async () => {
        ...
        try {
          ...
          if (err.code === 'ENOENT') {
            logger.debug('JSONL file removed during watcher callback', { module: 'watchers', sessionId: entry.sessionId.substring(0, 8) });
            return;
          }
          logger.error('JSONL watcher callback error', { module: 'watchers', op: 'startJsonlWatcher', err: err.message });
        }
      }, 500));
    });
  }
    ...
    fs.watchFile(settingsPath, { persistent: false, interval: 5000 }, async () => {
      try {
        ...
        if (err.code === 'ENOENT') {
          ...
          logger.warn('Settings file contains invalid JSON', { module: 'watchers', op: 'startSettingsWatcher' });
        } else {
          logger.error('Settings watcher error', { module: 'watchers', op: 'startSettingsWatcher', err: err.message });
        }
      }
    });
  ...
  function startCompactionMonitor() {
    ...
    compactionMonitorInterval = setInterval(async () => {
      try {
        for (const dbProj of db.getProjects()) {
          ...
          try {
            ...
            if (err.code !== 'ENOENT') {
              logger.error('Compaction monitor error scanning sessions', { module: 'watchers', op: 'startCompactionMonitor', err: err.message });
            }
            /* expected for ENOENT: sessions dir does not exist for this project */
          }
        }
      } catch (err) {
        logger.error('Compaction monitor fatal error', { module: 'watchers', op: 'startCompactionMonitor', err: err.message });
      }
  ...
  async function registerMcpServer() {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        logger.error('settings.json is corrupt — cannot register MCP server without overwriting user config', { module: 'watchers', op: 'registerMcpServer' });
        return;
      } else {
        logger.warn('Failed to read settings.json for MCP', { module: 'watchers', op: 'registerMcpServer', err: err.message });
      }
    }
  ...
  async function trustProjectDirs() {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        logger.error('.claude.json is corrupt JSON — skipping trustProjectDirs to preserve file for inspection', { module: 'watchers', op: 'trustProjectDirs' });
        return;
      } else {
        logger.warn('Failed to read .claude.json', { module: 'watchers', op: 'trustProjectDirs', err: err.message });
      }
    }
  ...
  async function ensureSettings() {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        logger.error('Unexpected error checking settings file', { module: 'watchers', op: 'ensureSettings', err: err.message });
      }
    }
  }
