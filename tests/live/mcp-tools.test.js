...
test('MCP-06g/06f: add task and get tasks via MCP', async () => {
  ...
  const addResult = await post('/api/mcp/call', { tool: 'blueprint_add_task', args: { project: 'mcp_proj', text: 'mcp-task-test' } });
  assert.ok(addResult.data.result);
  const r = await post('/api/mcp/call', { tool: 'blueprint_get_tasks', args: { project: 'mcp_proj' } });
