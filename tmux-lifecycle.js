...
module.exports = function createTmuxLifecycle({ safe, MAX_TMUX_SESSIONS, TMUX_CLEANUP_DELAY, logger }) {
  ...
  function scheduleTmuxCleanup(tmuxSession) {
    ...
    const timer = setTimeout(async () => {
      ...
      if (await safe.tmuxExists(tmuxSession)) {
        ...
        logger.info('Killed idle tmux session', { module: 'tmux-lifecycle', op: 'scheduleTmuxCleanup', tmuxSession });
      }
    }, TMUX_CLEANUP_DELAY);
  ...
  async function enforceTmuxLimit() {
    try {
      const stdout = await safe.tmuxExecAsync(['list-sessions', '-F', '#{session_name} #{session_activity}']);
      ...
      if (err.message && (err.message.includes('no server running') || err.message.includes('error connecting to'))) {
        ...
        logger.warn('enforceTmuxLimit: error enforcing limits', { module: 'tmux-lifecycle', err: err.message });
      }
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (err.code !== 'ENOENT') {
            logger.debug('cleanBridgeFiles: could not stat/unlink file', { module: 'tmux-lifecycle', file, err: err.message });
          }
          /* expected: file removed between readdir and stat */
        }
      }
      if (cleaned > 0) logger.info('Cleaned up old bridge files on startup', { module: 'tmux-lifecycle', count: cleaned });
    } catch (err) {
      if (err.code === 'ENOENT') {
        /* expected: bridge directory does not exist yet on first run */
      } else {
        logger.error('cleanBridgeFiles: bridge dir read error', { module: 'tmux-lifecycle', err: err.message });
      }
    }
  }
