      ...
      for (let i = 0; i < maxAttempts; i++) {
        ...
        try {
          ...
          if (newFiles.length >= 1) {
            ...
            logger.info('Session ID resolved', { module: 'session-resolver', from: tmpId.substring(0, 12), to: realId.substring(0, 8) });
          ...
          if (err.code === 'ENOENT') {
            if (i === 0) {
              logger.info('Waiting for sessions dir / JSONL to appear', { module: 'session-resolver', tmpId: tmpId.substring(0, 12) });
            }
            /* expected: sessions directory not created yet by claude CLI */
          } else {
            logger.error('Error scanning sessions dir', { module: 'session-resolver', op: 'resolveSessionId', err: err.message });
          }
        }
      }

      logger.warn('Session resolution timed out — JSONL never appeared', { module: 'session-resolver', tmpId: tmpId.substring(0, 12), projectId });
      if (!(await tmuxExists(tmux))) {
        db.deleteSession(tmpId);
        logger.info('Cleaned up orphaned temp session (tmux dead)', { module: 'session-resolver', tmpId: tmpId.substring(0, 12) });
      } else {
        logger.info('tmux still running; leaving temp session for later resolution', { module: 'session-resolver', tmpId: tmpId.substring(0, 12) });
      }
    } finally {
      ...
      try {
        ...
        if (err.code === 'ENOENT') {
          ...
          for (const stale of staleSessions) {
            logger.info('Startup: removing orphaned temp session (no sessions dir)', { module: 'session-resolver', staleId: stale.id.substring(0, 15) });
            db.deleteSession(stale.id);
          }
        } else {
          logger.error('Cannot read sessions dir for stale check', { module: 'session-resolver', op: 'resolveStaleNewSessions', err: err.message });
        }
        continue;
      }
      ...
      for (const stale of staleSessions) {
        if (unresolvedJsonls.length > 0) {
          ...
          if (db.getSession(realId)) {
            logger.info('Startup: real session already exists, removing stale entry', { module: 'session-resolver', staleId: stale.id.substring(0, 15), realId: realId.substring(0, 8) });
            db.deleteSession(stale.id);
            continue;
          }

          logger.info('Startup: resolved stale session', { module: 'session-resolver', from: stale.id.substring(0, 15), to: realId.substring(0, 8) });
          ...
          logger.info('Startup: removing orphaned temp session', { module: 'session-resolver', staleId: stale.id.substring(0, 15) });
          db.deleteSession(stale.id);
        }
      }
    }
