...
const ADMIN_TOOLS = [
  ...
  { name: 'blueprint_set_session_state', description: 'Change session state: active, archived, or hidden.' },
...
function registerExternalMcpRoutes(app) {
  ...
  app.get('/api/mcp/external/tools', async (req, res) => {
    try {
      ...
      if (!internalRes.ok) {
        logger.warn('Internal MCP tools endpoint returned error', { module: 'mcp-external', status: internalRes.status });
        return res.json({ tools: ADMIN_TOOLS });
      }
      const internal = await internalRes.json();
      res.json({ tools: [...(internal.tools || []), ...ADMIN_TOOLS] });
    } catch (err) {
      logger.error('Failed to fetch internal MCP tools', { module: 'mcp-external', err: err.message });
  ...
  app.post('/api/mcp/external/call', async (req, res) => {
    ...
    if (internalTools.includes(tool)) {
      try {
        ...
        logger.error('Internal MCP call network error', { module: 'mcp-external', tool, err: err.message });
    ...
    try {
      ...
      switch (tool) {
        case 'blueprint_create_session': {
          ...
          if (args.project.length > PROJECT_NAME_MAX_LEN) return res.status(400).json({ error: `project name too long (max ${PROJECT_NAME_MAX_LEN})` });
          if (args.model && !MODEL_PATTERN.test(args.model)) return res.status(400).json({ error: 'invalid model name' });
        ...
        case 'blueprint_set_session_state':
          ...
          if (!VALID_STATES.includes(args.state)) return res.status(400).json({ error: `state must be one of: ${VALID_STATES.join(', ')}` });
        ...
        case 'blueprint_set_project_claude_md': {
          ...
          if (args.content.length > 100000) return res.status(400).json({ error: 'content too long (max 100000)' });
        ...
        case 'blueprint_update_settings':
          ...
          if (!SETTINGS_KEY_PATTERN.test(args.key)) return res.status(400).json({ error: 'invalid settings key format' });
      ...
      logger.error('MCP external tool call error', { module: 'mcp-external', tool, err: err.message });
